...
% Load model
model = mphload('C:\Users\ECHOI\Downloads\c_20_ocv2.mph'); % ocv2 model

ModelUtil.showProgress(true);

%% OCV plot

SOC_ocv = mphglobal(model,'SOC','dataset','dset1');
OCV_ocv = mphglobal(model,'OCV','dataset','dset1');

C_rate_OCV = 0.05; % 


model.param.set('C_rate', num2str(C_rate_OCV));



%% Load data
data_folder = 'G:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\OCV\es_ex_1C\OCV2'; 
...
for i = 1:length(C_rates)
    ...
    plot(data_multi(i).SOC, data_multi(i).V, '-', 'DisplayName', sprintf('Experimental Data C=%d', C_rates(i)));
    
    % Plot optimized model
    plot(fit_result(i).SOC_fit, fit_result(i).V_fit, '--', 'DisplayName', sprintf('Optimized Model C=%d', C_rates(i)));
end
plot(SOC_ocv,OCV_ocv,'--','DisplayName',sprintf('%dC',C_rate_OCV))
xlabel('SOC');
ylabel('Voltage (V)');
legend;
title('Multi fitting')
